...
class AddIncom extends StatefulWidget {
  const AddIncom({Key? key}) : super(key: key);
  @override
  State<AddIncom> createState() => _AddIncomState();
}

class _AddIncomState extends State<AddIncom> {
  ...
  add_Incom() async {
    ...
    if (data["result"] == "done") {
      ...
      print("done");
      totalincom = (int.parse(totalincom) + int.parse(price.text)).toString();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: myAppBar("اضافة دخل ", Icon(Icons.switch_account)),
      body: Column(
        key: formstate,
        children: [
          SizedBox(height: 60),
